...
if (!empty($liet_id) && !empty($tb_name) && !empty($item_id)) { // проверяем, что переменные заполнены
    // Проверяем, существует ли уже этот элемент в таблице
    ...
    if ($result->num_rows > 0) {
        ...
        if ($delete_query->execute()) {
            ...
            $response['star_icon'] = 'far fa-star fav'; // Иконка после удаления
        } else {
            $response['success'] = false;
            $response['error'] = "Kļūda izņemšanā no favorītiem.";
        }
    } else {
        // Если записи нет, добавляем в таблицу
        ...
        if ($insert_query->execute()) {
            ...
            $response['star_icon'] = 'fa fa-star fav'; // Иконка после добавления
        } else {
            $response['success'] = false;
